...
for vn_str = {'overlaid_vel_null','bilat_vel_null'}
    ...
    for vel_set = 1:5
        ...
        for stim_ind = stim_inds
            graph.line{1}(iter) = summ_data.(vel_null_type_str)(exp_grp_num).flip.avg_lmr{stim_ind};
            graph.shade{1}(iter) = summ_data.(vel_null_type_str)(exp_grp_num).flip.sem_lmr{stim_ind};    
            iter = iter + 1;
        end
        graph.color{1} = [0 0 0];
        ...
        plot(vel_null.(vel_null_type_str)(1).fit_line(vel_set,:),'Color',[1 0 .5],'LineWidth',1);
    ...
    lp=semilogx(test_temp_freq_values,vel_null.(vel_null_type_str)(1).null_contrast','Color',[1 0 .5]);
